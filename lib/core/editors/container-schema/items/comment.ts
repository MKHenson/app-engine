namespace Animate {
    ...
    export class Comment extends CanvasItem {
        ...
        constructor( label: string = 'Type a message' ) {
            super({ type: 'comment', label: label } as Engine.Editor.IComment);
            // this.label = label;
            // this.width = 150;
            // this.height = 50;
        }
        ...
        clone( clone?: Comment ) : Comment {
            if ( !clone )
                clone = new Comment( this.serializer.get('label') );
        ...
        //     this._immutable = this._immutable.merge( { id: id } as Engine.Editor.IComment );
        //     return
