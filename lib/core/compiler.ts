    ...
    export class Compiler {
        ...
        */
        public static cloneNode( node: AppNode ): Node {
            const clone: AppNode = <AppNode>node.cloneNode( false );
        ...
        */
        static digestCSS( elm: AppNode, controller: any, value: string ) {
            const object = Compiler.parse( value, controller, null, elm, null );
            const htmlElem = <HTMLElement><Node>elm;
        ...
        */
        static isEquivalent( a: any, b: any ): boolean {
            ...
            for ( let i = 0; i < aProps.length; i++ ) {
                ...
                const aVal = a[ aPropName ];
                const bVal = b[ bPropName ];
                ...
                if ( null === aVal || 'object' !== typeof aVal ) {
                    ...
                    for ( let ai = 0, al = aVal.length; ai < al; ai++ )
        ...
        */
        static removeEvents( elem: Element ) {
            const appNode = <AppNode><Node>elem;
            if ( appNode.$events ) {
                for ( let i = 0, l = appNode.$events.length; i < l; i++ )
        ...
        static expand( root: RootNode, ctrl: any, includeSubTemplates: boolean = false ): Element {
            ...
            Compiler.traverse( root, function( child: Node ) {
                ...
                if ( ( <RootNode><Node>child ).$ctrl ) {
                    ...
                    for ( const i in subRoot.$commentReferences )
                ...
                if ( commentElement && commentElement.$originalNode.$expression ) {
                    const $expression = commentElement.$originalNode.$expression;
                    const $expressionType = commentElement.$originalNode.$expressionType;
                    ...
                    if ( Compiler.directives[ $expressionType ] ) {
                        ...
                        const results = Compiler.directives[ $expressionType ].expand( $expression, ctrl, commentElement, <InstanceNode>child );
                        let newNode: AppNode;
                        ...
                        for ( let c = 0, k = ( <InstanceNode>child ).$clonedElements.length; c < k; c++ )
                        ...
                        for ( let k = 0, lk = results.length; k < lk; k++ ) {
                            newNode = results[ k ];
        ...
        */
        static registerFunc( node: AppNode, name: string, tag: string, func: any ) {
            ...
            for ( let i = 0, l = node.$events.length; i < l; i++ )
        ...
        */
        static digest( jElem: JQuery, controller: any, includeSubTemplates: boolean = false ): Element {
            ...
            // const rootNode: RootNode = <RootNode><Node>elm;
            ...
            //     const jElemWrapper = jQuery( elem );

            //     // If a text node
            //     if ( elem.nodeType === 3 ) {
            //         textNode = <AppNode><Node>elem;
            //         let origText = '';
            //         const dataExpression = textNode.$expression;
            ...
            //         const parsedText = origText.replace( /\{\{(.*?)\}\}/g, function ( sub, val ) {
            ...
            //     const appNode = <AppNode><Node>elem;
            //     const attrs: Array<Attr> = Compiler.attrs;
            //     attrs.splice( 0, attrs.length );
            //     for ( let i = 0; i < ( <Element>elem ).attributes.length; i++ )
            ...
            //         switch ( name ) {
            //             // case 'en-src':
            //             //     const src = Compiler.parse( value, controller, null, elem, null );
            ...
            //             //     break;
            //             // case 'en-show':
            //             //     const disp = ( Compiler.parse( value, controller, null, elem, null ) ? '' : 'none' );
            ...
            //             //     break;
            //             // case 'en-html':
            //             //     const html = Compiler.parse( value, controller, null, elem, null );
            //             //     ( <HTMLElement>elem ).innerHTML = html;
            //             //     break;
            //             // case 'en-scrolltop':
            //             //     let val = Compiler.parse( value, controller, null, elem, null );
            //             //     if ( val !== undefined )
            ...
            //             //     break;
            //             // case 'en-href':
            //             //     const href = Compiler.parse( value, controller, null, elem, null );
            ...
            //             //     const val = Compiler.parse( `${value}`, controller, null, elem, null );
            ...
            //             //     const ev = function ( e ) {
            //             //         const form: NodeForm = <NodeForm>elem;
            //             //         e.preventDefault();
            //             //         form.$error = false;
            ...
            //             //             let err = Compiler.checkValidations( this.getAttribute( 'en-validate' ), <HTMLInputElement>subElem );
            //             //             if ( err ) {
            //             //                 form.$error = true;
            //             //                 form.$pristine = false;
            //             //             }
        ...
        static validateNode( elem: NodeInput ) {
            // IF it has a form - check other elements for errors
            const form: NodeForm = <NodeForm>elem.form;
            if ( form ) {
                form.$error = false;
                form.$errorInput = '';
            }
        ...
        */
        static checkValidations( value: string, elem: HTMLInputElement | HTMLTextAreaElement ): boolean {
            const expressions: Array<{ name: string; regex: RegExp; negate: boolean; }> = [];
            const form: NodeForm = <NodeForm>elem.form;

            for ( let i = 0, values = value.split( '|' ), l = values.length; i < l; i++ )
            ...
            for ( let i = 0, l = expressions.length; i < l; i++ ) {
                let matches: any = elem.value.match( expressions[ i ].regex );
        ...
        */
        static build( elm: JQuery, ctrl: any, includeSubTemplates: boolean = false ): JQuery {
            const rootNode = <RootNode><Node>elm.get( 0 );
            rootNode.$ctrl = ctrl;

            rootNode.$commentReferences = {};

            const potentials: Array<Element> = [];
            ...
            for ( let i = 0, l = potentials.length; i < l; i++ ) {
                Compiler.$commentRefIDCounter++;
                const comment = jQuery( `<!-- ${( <AppNode><Node>potentials[ i ] ).$expressionType}[${Compiler.$commentRefIDCounter}] -->` );
                const commentElement: DescriptorNode = <DescriptorNode><Node>comment.get( 0 );
