...
import { PropertyType, ResourceType } from '../setup/enums';
import { TreeNodeGroupInstance } from '../components/treeview/nodes/treenode-group-instance';
...
export class ImportExport extends EventDispatcher {
    ...
    * @returns {any}
    */
    referenceCheckAsset( asset: Asset, container: any ) {
        if ( asset === null )
            return;
        ...
        for ( let i = 0, l = assetVars.length; i < l; i++ ) {
            if ( assetVars[ i ].type === PropertyType.ASSET ) {
                const asset = <Asset>assetVars[ i ].getVal();
                if ( asset ) {
                ...
                const aList = <Array<Asset>>assetVars[ i ].getVal();

                for ( let a = 0, al = aList.length; a < al; a++ ) {
                    const asset = <Asset>aList[ a ];
                    container.assets.push( asset.entry.shallowId );
                ...
                const group = <GroupArray>assetVars[ i ].getVal();
