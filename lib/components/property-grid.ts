...
import { PGAssetList } from './property-editors/pg-asset-list';
import { PGColorPicker } from './property-editors/pg-color-picker';
import { PGComboAsset } from './property-editors/pg-combo-asset';
import { PGComboBool } from './property-editors/pg-combo-bool';
import { PGComboEnum } from './property-editors/pg-combo-enum';
import { PGComboGroup } from './property-editors/pg-combo-group';
import { PGFile } from './property-editors/pg-file';
import { PGNumber } from './property-editors/pg-number';
import { PGTextbox } from './property-editors/pg-textbox';
...
export class PropertyGrid extends Component {
    ...
    */
    cleanup() {
        // Cleanup editors
        for ( let i = 0, l = this._editors.length; i < l; i++ )
            this._editors[ i ].cleanup();

        // Cleanup groups
        for ( let i = 0, l = this._groups.length; i < l; i++ ) {
    ...
    */
    editableObject( object: EditableSet | null, name: string, img: string = '' ) {
        ...
        if ( object !== undefined && object !== null ) {
            ...
            const variables: Array<Prop<any>> = object.variables;
            ...
            for ( let i = 0; i < sortable.length; i++ ) {
                const editors: Array<PropertyGridEditor> = this._editors;
                for ( let editor = 0, el = editors.length; editor < el; editor++ ) {
