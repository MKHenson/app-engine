...
import { SelectionMoved } from '../../../core/editors/container-schema/actions/selection-moved';
import { CommentResized } from '../../../core/editors/container-schema/actions/comment-resized';
import { CommentEditted } from '../../../core/editors/container-schema/actions/comment-editted';
...
export class CommentComponent extends React.Component<ICommentComponentProps, ICommentComponentState> {
    ...
    onUp( e: React.MouseEvent ) {
        ...
        window.removeEventListener( 'mouseup', this._onUp );
        this.props.editor.doAction( new CommentEditted( this.props.comment.id!, input.value ) );
        ...
        return (
            <Draggable
                ...
                onDragComplete={( start, end ) => {
                    ...
                    editor.doAction( new SelectionMoved( [ { index: comment.id!, x: end.x, y: end.y }] ) );
                } }>
                <Resizable
                    onResized={( size ) => { editor.doAction( new CommentResized( comment.id!, size.width, size.height ) ) } }
                        ...
                        {this.state.editMode ?
                            <textarea ref="input"
                                ...
                                onKeyDown={( e ) => {
                                    ...
                                    if ( e.keyCode === 13 ) {
                                        this.setState( { editMode: false });
                                        editor.doAction( new CommentEditted( this.props.comment.id!, ( e.target as HTMLTextAreaElement ).value ) );
                                    }
                                } }
